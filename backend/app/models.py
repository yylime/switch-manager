...
from sqlmodel import Field, Relationship, SQLModel, UniqueConstraint, desc, Index, Column, JSON
...
class SwitchBase(SQLModel):
    ...
    port: int = Field(default=22, description="管理端口")
...
class SwitchConfig(SwitchConfigBase, table=True):
    __table_args__ = (
        ...
        Index("idx_switch_status_created", "switch_id", "status", "created")
    )
    id: uuid.UUID = Field(default_factory=uuid.uuid4, primary_key=True)
...
class EverydayStatisticsBase(SQLModel):
    add_date: date = Field(default_factory=date.today, description="统计日期")
    vrf_count: int = Field(default=0, description="VRF数量")
    vrf_different_count: int = Field(default=0, description="昨日相比VRF数量")
    ip_count: int = Field(default=0, description="IPtables数量")
    ip_different_count: int = Field(default=0, description="昨日相比IP数量")
    switch_different_count: int = Field(default=0, description="不同配置的交换机数量")

class EverydayStatistics(EverydayStatisticsBase, table=True):
    id: uuid.UUID = Field(default_factory=uuid.uuid4, primary_key=True)


# ================== Backup Scheduler ===================
class BackupScheduler(SQLModel, table=True):

    id: uuid.UUID = Field(default_factory=uuid.uuid4, primary_key=True)
    job_id: str = Field(index=True, unique=True)  # APScheduler job id
    name: str
    job_type: str  # full_backup / failed_backup
    cron: str  # cron 表达式: "0 2 * * *"
    enabled: bool = True
...
# ================== BackupTask ==================
class BackupTaskBase(SQLModel):
    task_type: str = Field(max_length=50, description="任务类型: backup_all_switches 或 backup_failed_switches")
    status: str = Field(default="running", description="任务状态: running, success, failed")
    message: str = Field(default="", max_length=500, description="任务结果消息")
    error: str | None = Field(default=None, description="错误信息")
    start_time: datetime = Field(default_factory=datetime.now, description="任务开始时间")
...
class BackupTaskUpdate(SQLModel):
...
class BackupTask(BackupTaskBase, table=True):
    id: uuid.UUID = Field(default_factory=uuid.uuid4, primary_key=True)
    created_at: datetime = Field(default_factory=datetime.now, description="记录创建时间")


class BackupTaskPublic(BackupTaskBase):
    id: uuid.UUID
    created_at: datetime
...
class BackupTasksPublic(SQLModel):
    data: list[BackupTaskPublic]
    count: int
