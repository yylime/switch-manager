...
from app.services.switches.config.stype_detect import paramiko_detect_stype
from app.services.switches.config.utils import check_port
from app.services.switches.config.config_textfsm import read_switch_sn, read_switch_type, read_switch_version
from app.services.switches.error_handdle import netmiko_error_to_str


def updated_switch_hostname(switch: Switch, output: str) -> bool:
    if output != "null":
        # 正则表达式读取配置中的文件名
...
def check_updated_switch_stype(switch: Switch, session: Session, res: dict) -> bool:
    ...
    if "name" not in output:
        ...
        switch.stype = paramiko_detect_stype(
            switch.ip, switch.inspector.name, switch.inspector.password, login_type=switch.login_type.name, port=switch.port
        )
...
def read_config(switch: Switch, session: Session, update_force=False) -> dict:
    ...
    try:
        with ConnectHandler(
            ...
            # 7. 更新交换机名称
            updated_switch_hostname(switch, output)
            session.commit()  # 保存所有改动
            return {"status": "success", "output": output}
    except Exception as e:
        return {"status": netmiko_error_to_str(e), "output": "null"}
...
def download_config(switch_id: str) -> str:
    ...
    with db_session() as session:
        ...
        if switch is None:
            return "no_switch"
        if check_port(switch.ip, port=22 if switch.login_type.name == "ssh" else 23):
    
            # default the configure is null
            res = read_config(switch=switch, session=session)

            # 发现设备被替换，则进行一次更新交换机的设备类型，然后重新读取配置
        ...
        cur_cfg.status = (
            "success" if res["status"] == "success" else res["status"][:20]
        )
