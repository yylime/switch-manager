...
def list_backup_jobs(session: SessionDep, current_user: CurrentUser):
...
@router.put("/{id}")
def update_backup_job( session: SessionDep, request: BackupJobUpdate, id: uuid.UUID, current_user: CurrentUser):
...
@router.delete("/{id}")
def delete_backup_job(session: SessionDep, id: uuid.UUID, current_user: CurrentUser):
    job = session.get(BackupScheduler, id)
    if not job:
        raise HTTPException(404, "Job not found")
