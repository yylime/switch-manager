...
export type SwitchesQueryParams = {
  ...
  pageSize?: number
  searchText?: string
  branchIds?: Array<string>  // 修改为复数形式，与后端保持一致
  status?: Array<string>
}
...
export async function fetchSwitches(params: SwitchesQueryParams = {}) {
  ...
  const res = await SwitchesService.readSwitches({
    ...
    branchIds: params.branchIds ?? [],  // 修改为复数形式，与后端保持一致
...
export function useSwitches(params: SwitchesQueryParams = {}) {
  return useQuery({
    queryKey: ["switches", params.page ?? 1, params.pageSize ?? 10, params.searchText ?? "", params.branchIds ?? [], params.status ?? []],  // 修改为复数形式
