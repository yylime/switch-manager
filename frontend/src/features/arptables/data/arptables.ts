...
import { ArptablesService } from "@/client"
...
export async function fetchIptables(params: itemsQueryParams = {}) {
  ...
  const res = await ArptablesService.readArptables({
    skip,
    limit: pageSize,
...
export function useArptables(params: itemsQueryParams = {}) {
  return useQuery({
    queryKey: ["arptables", params.page ?? 1, params.pageSize ?? 10, params.searchText ?? ""],
